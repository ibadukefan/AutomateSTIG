...
class GenerateRuleImplementationSpecsTests(unittest.TestCase):
    ...
    def test_infers_legacy_temporary_account_expiration_candidates_from_expiration_date_prose(self):
        cases = [
            ...
            ('V-248651', 'oracle_linux_8_stig', 'OL 8'),
            ('V-238331', 'canonical_ubuntu_20_04_lts_stig', 'Ubuntu 20.04 LTS'),
            ('V-234866', 'sles_15_stig', 'SLES 15'),
        ]
        for vuln_id, stig_id, product in cases:
            with self.subTest(vuln_id=vuln_id):
                candidate = mod.infer_candidate_check({
                    'vuln_id': vuln_id,
                    'title': f'{product} temporary accounts must expire within 72 hours.',
                    'check_content': (
                        'Verify temporary accounts have been provisioned with an expiration date of 72 hours.\n\n'
                        'For every existing temporary account, run the following command to obtain its account expiration information.\n\n'
                    ...
                    'fix_text': (
                        'If a temporary account must be created, configure the system to terminate the account after a 72-hour time period.\n\n'
    ...
    def test_infers_ubuntu_2004_data_at_rest_crypttab_candidate(self):
        check = (
        ...
        self.assertIsNotNone(candidate)
        self.assertEqual(candidate['vuln_id'], 'V-238335')
        self.assertEqual(candidate['platform'], 'linux')
        self.assertEqual(candidate['check']['type'], 'command_output')
...
kubectl create -f restricted.yml''',
    ...
    def test_infers_kubernetes_api_server_tls_certificate_flags_candidate(self):
        candidate = mod.infer_candidate_check({
            'vuln_id': 'V-242422',
            'title': 'Kubernetes API Server must have a certificate for communication.',
            'check_content': 'Change to the /etc/kubernetes/manifests/ directory on the Kubernetes Control Plane. Run the command:\n\ngrep -i tls-cert-file * | grep -i tls-private-key-file *\n\nIf the setting tls-cert-file and private-key-file is not set in the Kubernetes API server manifest file or contains no value, this is a finding.',
            'fix_text': 'Edit the Kubernetes API Server manifest file in the /etc/kubernetes/manifests directory on the Kubernetes Control Plane. Set the value of tls-cert-file and tls-private-key-file to path containing API Server certificate and key.',
        }, 'Kubernetes_STIG')
        self.assertIsNotNone(candidate)
    ...
    def test_infers_oracle_profile_password_lock_time_unlimited_candidate(self):
        candidate = mod.infer_candidate_check({
            'vuln_id': 'V-270549',
            'title': 'Oracle Database must verify account lockouts persist until reset by an administrator.',
            'check_content': "From SQL*Plus: SQL>SELECT PROFILE, RESOURCE_NAME, LIMIT FROM DBA_PROFILES WHERE PROFILE = 'ORA_STIG_PROFILE'; Check the settings for password_lock_time - this specifies how long to lock the account after the number of consecutive failed logon attempts reaches the limit. If the value is not UNLIMITED, this is a finding.",
            'fix_text': 'Configure the database management system (DBMS) settings to specify indefinite lockout duration: ALTER PROFILE ORA_STIG_PROFILE LIMIT PASSWORD_LOCK_TIME UNLIMITED;',
        }, 'Oracle_Database_19c_STIG')
        self.assertIsNotNone(candidate)
        self.assertEqual(candidate['vuln_id'], 'V-270549')
        self.assertEqual(candidate['platform'], 'generic')
...
If the INACTIVE_ACCOUNT_TIME parameter is set to UNLIMITED (default) or it is set to more than 35 days, this is a finding.""",
    ...
    def test_infers_linux_usbguard_rules_present_for_exact_disa_vulns(self):
        cases = [
            ('V-230524', 'RHEL_8_STIG', '$ sudo usbguard list-rules'),
            ('V-258038', 'RHEL_9_STIG', '$ sudo usbguard list-rules\nallow id 1d6b:0001 serial'),
            ('V-271701', 'Oracle_Linux_9_STIG', '$ usbguard list-rules\nallow id 1d6b:0001 serial'),
            ('V-248863', 'Oracle_Linux_8_STIG', '$ sudo usbguard list-rules'),
        ]
        for vuln_id, stig_id, command_text in cases:
            with self.subTest(vuln_id=vuln_id):
                candidate = mod.infer_candidate_check({
                    'vuln_id': vuln_id,
                    'title': 'The operating system must block unauthorized peripherals before establishing a connection.',
                    'check_content': f'Verify the USBGuard has a policy configured with the following command:\n\n{command_text}\n\nIf the command does not return results or an error is returned, ask the SA to indicate how unauthorized peripherals are being blocked. If there is no evidence that unauthorized peripherals are being blocked before establishing a connection, this is a finding.',
                    'fix_text': 'Configure the USBGuard daemon with a policy that blocks unauthorized peripherals before establishing a connection.',
                }, stig_id)
                self.assertIsNotNone(candidate)
                self.assertEqual(candidate['vuln_id'], vuln_id)
                self.assertEqual(candidate['platform'], 'linux')
                self.assertEqual(candidate['check'], {'type': 'command_output', 'command': 'usbguard list-rules'})
                self.assertEqual(candidate['expected'], {'type': 'not_equals', 'value': ''})
...
If "flags" does not contain the "nx" flag, this is a finding.''',
    ...
    def test_infers_linux_vendor_supported_release_from_authoritative_sample(self):
        cases = [
            (
                'V-230221',
                'RHEL_8_STIG',
        ...
        for vuln_id, stig_id, title, command, sample_line, path, expected_line, finding_text in cases:
            with self.subTest(vuln_id=vuln_id):
                candidate = mod.infer_candidate_check({
                    'vuln_id': vuln_id,
                    'title': title,
                    'check_content': f'''Verify the version of the operating system is vendor supported.
...
{finding_text}''',
                    'fix_text': 'Upgrade to a supported version.',
                }, stig_id)
...
If the site includes any system areas such as root of the drive, Program Files, or Windows directories, this is a finding.''',
    ...
    def test_infers_ubuntu_2204_chrony_usno_maxpoll_candidate(self):
        candidate = mod.infer_candidate_check({
            'vuln_id': 'V-260519',
            ...
            'check_content': '''Verify Ubuntu 22.04 LTS is configured to compare the system clock at least every 24 hours to the authoritative time source by using the following command:
...
If "server" is not defined, is not set to an authoritative DOD time source, is commented out, or missing, this is a finding.''',
            'fix_text': 'Configure Ubuntu 22.04 LTS to compare the system clock at least every 24 hours to the authoritative time source. Add or modify the following line in the "/etc/chrony/chrony.conf" file: server [source] iburst maxpoll = 16',
        }, 'CAN_Ubuntu_22-04_LTS_STIG')
        self.assertEqual(candidate, {
            'vuln_id': 'V-260519',
            'platform': 'linux',
