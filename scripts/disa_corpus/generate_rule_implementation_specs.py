...
def _postgresql_audit_explicit_config_candidate(rule: dict, stig_id: str) -> dict | None:
    ...
    if vuln_id in denial_logging_vuln_ids:
        ...
        if not re.search(r'(?:If\s+(?:the\s+)?denials?\s+(?:is|are)\s+not\s+logged|If\s+audit\s+logs\s+are\s+not\s+generated\s+when\s+unsuccessful\s+attempts|If\s+audit\s+records\s+are\s+not\s+produced|If\s+any\s+of\s+the\s+above\s+steps\s+did\s+not\s+create\s+audit\s+records|If\s+the\s+above\s+steps\s+cannot\s+verify\s+that\s+audit\s+records\s+are\s+produced)', content, re.IGNORECASE):
            return None
        if not re.search(r'(?:All\s+(?:errors\s+and\s+)?denials\s+are|Unsuccessful\s+attempts\s+to\s+modify\s+security\s+objects\s+can\s+be)\s+logged\s+(?:by\s+default\s+)?if\s+logging\s+is\s+enabled', fix_text, re.IGNORECASE):
            return None
...
def _sql_server_windows_authentication_only_candidate(rule: dict, stig_id: str) -> dict | None:
    ...
    content = rule.get('check_content', '') or ''
    fix_text = rule.get('fix_text', '') or ''
    if rule.get('vuln_id', '') != 'V-271265':
        return None
    expected_title = 'sql server must integrate with an organization-level authentication/access mechanism providing account management and automation for all users, groups, roles, and any other principals.'
    if rule.get('title', '').strip().lower() != expected_title:
        return None
    required_content_patterns = (
...
def _iis_tls_12_enabled_legacy_protocols_disabled_candidate(rule: dict, stig_id: str) -> dict | None:
    vuln_id = rule.get('vuln_id', '')
    if stig_id != 'IIS_10-0_Server_STIG' or vuln_id not in {'V-218821', 'V-218822'}:
        return None
    content = rule.get('check_content', '') or ''
    fix_text = rule.get('fix_text', '') or ''
    combined = f'{content}\n{fix_text}'
    if vuln_id == 'V-218821':
        ...
        required_snippets = (
            r'TLS\s+version\s+is\s+not\s+TLS\s+1\.2\s+or\s+higher',
            r'non-FIPS-approved\s+algorithms\s+are\s+enabled',
            r'use\s+an\s+approved\s+TLS\s+version\s+according\s+to\s+NIST\s+SP\s+800-52',
            r'disable\s+all\s+non-approved\s+versions',
        )
        if not all(re.search(snippet, combined, re.IGNORECASE) for snippet in required_snippets):
            return None
        command = "powershell -NoProfile -Command \"$expected=@{'TLS 1.2'=@{Enabled=1;DisabledByDefault=0};'TLS 1.0'=@{Enabled=0;DisabledByDefault=1};'TLS 1.1'=@{Enabled=0;DisabledByDefault=1};'SSL 3.0'=@{Enabled=0;DisabledByDefault=1}}; foreach($protocol in $expected.Keys){$path='HKLM:\\SYSTEM\\CurrentControlSet\\Control\\SecurityProviders\\SCHANNEL\\Protocols\\'+$protocol+'\\Server'; $item=Get-ItemProperty -Path $path -ErrorAction SilentlyContinue; if(-not $item){exit 1}; foreach($name in $expected[$protocol].Keys){ if([int]$item.$name -ne [int]$expected[$protocol][$name]){exit 1}}}; $fips=Get-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Lsa\\FipsAlgorithmPolicy' -ErrorAction SilentlyContinue; if(-not $fips -or [int]$fips.Enabled -ne 1){exit 1}; 'Compliant'\""
    return {
        'vuln_id': vuln_id,
        'platform': 'windows',
        'check': {'type': 'command_output', 'command': command},
