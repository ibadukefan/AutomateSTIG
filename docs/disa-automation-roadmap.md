...
- conservative candidate check-template extraction from authoritative DISA prose for Windows registry, Windows Feature, Linux sysctl, Linux package, Linux file-content, Linux service-state, and Linux file-permission patterns;
...
- mapped automated checks after fixture-backed candidate and SCAP/manual canonical promotion: 2,159
  ...
  - generated candidate checks with deterministic pass/fail fixture evidence: 744
- remaining unsupported authoritative rules: 3,047
- generated planned specs for remaining unsupported rules: 3,047

The corpus grew again in this pass, from 14 to 22 authorized public DISA artifacts and from 3,828 to 5,206 authoritative rules. Additional Linux service-state and file-permission templates plus new Ubuntu/Oracle Linux/Chrome corpus coverage moved mapped/automated rules from 1,794 to 2,159, but the unsupported count increased from 2,034 to 3,047 because newly tracked artifacts added more authoritative rules than the latest templates could cover. This is still not production-complete or all-DISA complete. The production target remains zero unsupported rules across the full tracked public DISA corpus, with every rule classified as machine-verifiable automated, scanner-import-verifiable, automated manual-evidence workflow, or not-applicable-with-proof.
