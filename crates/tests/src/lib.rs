...
fn make_checklist_with_findings() -> Checklist {
    ...
    engine.evaluate(&benchmark, &asset, Some(&scan), &[]).unwrap()
}
...
fn test_database_full_lifecycle() {
    ...
    let cl2 = {
        ...
        let engine = EvaluationEngine::with_defaults();
        engine.evaluate(&b, &Asset::new("linuxbox"), None, &[]).unwrap()
    };
...
fn test_generate_answer_template_from_checklist() {
    ...
    let template =
        automatestig_core::answer::generate_answer_template(&checklist, false);
...
<Benchmark xmlns="http://checklists.nist.gov/xccdf/1.2" id="Test_Bench">
  ...
  <Group id="V-1002">
    ...
    assert!(benchmark.rules[0].cci_refs.contains(&"CCI-000001".to_string()));
...
!
    ...
    let r4 = evaluate_config_check(&config, "V-NET-004", &["ip access-list extended"], &[], None);
